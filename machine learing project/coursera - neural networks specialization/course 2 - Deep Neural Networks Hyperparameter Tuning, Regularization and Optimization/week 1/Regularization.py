...
def backward_propagation_with_regularization(X, Y, cache, lambd):
    ...
    ### START CODE HERE ### (approx. 1 line)
    dW3 = 1./m * np.dot(dZ3, A2.T) + ((lambd/m) * W3) #derivation of (lambd/(2*m))*(W^2)
    ### END CODE HERE ###
    db3 = 1./m * np.sum(dZ3, axis=1, keepdims = True)
    
    dA2 = np.dot(W3.T, dZ3)
    dZ2 = np.multiply(dA2, np.int64(A2 > 0)) 
    # np.int64(A2 > 0) - derviation of relu: 
    #if (x > 1) then func is linear line so f' = 1, 
    #if (x =< 1) then the func is a horizontal line so f' = 0
    ### START CODE HERE ### (approx. 1 line)
